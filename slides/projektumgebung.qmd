---
title: Projektumgebung
lang: de
subtitle: Einrichtung und Grundlagen
author: Data Science, AI & Analytics Toolkit
execute:
  eval: false
  echo: true
highlight-style: github-dark
format:
  revealjs: 
    toc: false
    toc-depth: 1
    embed-resources: false
    theme: [default, style/custom.scss]
    incremental: true
    transition: none
    background-transition: fade
    transition-speed: slow
    code-copy: true
    code-line-numbers: false
    smaller: false
    scrollable: true
    slide-number: false
    preview-links: auto
    css: style/index.css
    chalkboard: 
      buttons: false
    logo: images/code-blue.png
    footer: Jan Kirenz
    template-partials:
      - title-slide.html 
title-slide-attributes:
  data-background-color: "#ffffff"
jupyter: python3
---

## Inhalte des Tutorials

:::: {.features}

::: {.feature .fragment .fade-in}
:::{.feature-header}
Projektstruktur 🗂️
:::
Ordnerstruktur anlegen
:::

::: {.feature .fragment .fade-up}
:::{.feature-header}
Befehlszeile & Shell 💻
:::
Grundlagen verstehen
:::

::: {.feature .fragment .fade-up}
:::{.feature-header}
Betriebssysteme 🖥️
:::
Windows vs. macOS
:::

::::

## Projektstruktur

::: {layout-ncol=2 layout-valign="bottom"}

![Visuelle Darstellung](../images/ordner.png){width=45% fig-align="left"}

![Baumstruktur](../images/ordner2.png){width=55% fig-align="left"}

:::

## Baumstruktur

. . .

```plaintext
toolkit-lab/
- code/
- data/
```

## Befehlszeile vs. GUI

- Befehlszeile: Textbasierte Interaktion mit dem Betriebssystem
- GUI: Grafische Benutzeroberfläche (z.B. Windows Explorer, macOS Finder)

::: {.callout-note appearance="minimal"}
Die Befehlszeile ermöglicht effizientere Aufgabenausführung als eine GUI.
:::

## Bezeichnungen der Befehlszeile

- Kommandozeile
- Befehlszeilenschnittstelle
- Command Line Interface (CLI)

::: {.fragment .fade-in}
Windows: Eingabeaufforderung
:::

::: {.fragment .fade-in}
macOS: Terminal
:::

## Befehlszeile in verschiedenen Betriebssystemen

::: {layout-ncol=2 layout-valign="bottom"}

![Windows Eingabeaufforderung](../images/eingabe.png){width=70% fig-align="center"}

![macOS Terminal](../images/terminal.png){width=100% fig-align="center"}

:::

## Shell-Konzept

- Shell: Vermittler zwischen Benutzer und Betriebssystem
- Interpretiert Eingaben und übersetzt sie in Systemanweisungen

::: {.fragment .fade-in}
Befehlszeile = "Blatt Papier" für Anweisungen
:::

::: {.fragment .fade-in}
Shell = "Übersetzer" der Anweisungen
:::

## Shells in verschiedenen Betriebssystemen

::: {.panel-tabset}

### Windows

- Standardprogramm: Eingabeaufforderung (Command Prompt)
- Standardshell: cmd.exe
- Alternative: PowerShell (leistungsfähiger)

### macOS

- Programm: Terminal
- Aktuelle Standardshell: zsh (Z Shell)
- Frühere Standardshell (vor Sept. 2019): bash (Bourne Again Shell)

:::

## Bedeutung der Befehlszeile

- Wichtige Fähigkeit in Datenanalyse und Softwareentwicklung
- Ermöglicht präzise Kontrolle über das System
- Nützlich für Automatisierungen

## Zusammenfassung

:::: {.features}

::: {.feature .fragment .fade-in}
:::{.feature-header}
Projektstruktur 🗂️
:::
toolkit-lab/
  - code/
  - data/
:::

::: {.feature .fragment .fade-up}
:::{.feature-header}
Befehlszeile & Shell 💻
:::
Textbasierte Interaktion
Effizienz & Kontrolle
:::

::: {.feature .fragment .fade-up}
:::{.feature-header}
Betriebssysteme 🖥️
:::
Windows: Eingabeaufforderung
macOS: Terminal
:::

::::